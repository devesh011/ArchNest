export const HOSTING_CONFIG_KEY = "archnest_hosting_config";
...
export const createHostingSlug = () =>
  `archnest-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
...
export const getSubdomainFromHostedUrl = (url: unknown): string | null => {
  if (typeof url !== "string" || !url) return null;
  try {
    const hostname = new URL(url).hostname;
    if (!hostname.endsWith(HOSTING_DOMAIN_SUFFIX)) return null;
    return hostname.slice(0, -HOSTING_DOMAIN_SUFFIX.length);
  } catch {
    return null;
  }
};
